...
func getReplicaCount(logger logr.Logger, currentReplicas, currentReadyReplicas int32, wpa *v1alpha1.WatermarkPodAutoscaler, name string, adjustedUsage float64, lowMark, highMark *resource.Quantity) (replicaCount int32, utilization int64) {
	...
	adjustedHM := float64(highMark.MilliValue() + wpa.Spec.Tolerance.MilliValue()*highMark.MilliValue()/100)
	adjustedLM := float64(lowMark.MilliValue() - wpa.Spec.Tolerance.MilliValue()*lowMark.MilliValue()/100)
	...
	case adjustedUsage > adjustedHM:
		...
		logger.Info("Value is above highMark", "usage", utilizationQuantity.String(), "replicaCount", replicaCount, "currentReadyReplicas", currentReadyReplicas, "tolerance (%):", wpa.Spec.Tolerance.MilliValue(), "adjustedHM", adjustedHM, "adjustedUsage", adjustedUsage)
	case adjustedUsage < adjustedLM:
		...
		logger.Info("Value is below lowMark", "usage", utilizationQuantity.String(), "replicaCount", replicaCount, "currentReadyReplicas", currentReadyReplicas, "tolerance (%):", wpa.Spec.Tolerance.MilliValue(), "adjustedHM", adjustedHM, "adjustedLM", adjustedLM, "adjustedUsage", adjustedUsage)
	default:
		restrictedScaling.With(labelsWithReason).Set(1)
		value.With(labelsWithMetricName).Set(adjustedUsage)
		logger.Info("Within bounds of the watermarks", "value", utilizationQuantity.String(), "replicaCount", replicaCount, "currentReadyReplicas", currentReadyReplicas, "tolerance (%):", wpa.Spec.Tolerance.MilliValue(), "adjustedHM", adjustedHM, "adjustedUsage", adjustedUsage)
